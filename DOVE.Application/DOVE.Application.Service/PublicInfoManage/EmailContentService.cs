    ...
    public class EmailContentService : RepositoryFactory<EmailContentEntity>, IEmailContentService
    {
        ...
        public IEnumerable<EmailContentEntity> GetUnreadMail(Pagination pagination, string userId, string keyword)
        {
            ...
            strSql.Append(@"SELECT  a.AddresseeId AS ContentId ,
                            ...
                            WHERE   a.RecipientId = " + DbParameters.CreateDbParmCharacter() + "userId AND a.IsRead <> 1");
            ...
            if (!keyword.IsEmpty())
            {
                strSql.Append(" AND c.Theme like " + DbParameters.CreateDbParmCharacter() + "Theme");
                parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "Theme", '%' + keyword + '%'));
            }
        ...
        public IEnumerable<EmailContentEntity> GetAsteriskMail(Pagination pagination, string userId, string keyword)
        {
            ...
            strSql.Append(@"SELECT  a.AddresseeId AS ContentId ,
                            ...
                            FROM    Email_Addressee a
                                    LEFT JOIN Email_Content c ON c.ContentId = a.ContentId
                            WHERE   a.RecipientId = " + DbParameters.CreateDbParmCharacter() + "userId AND a.IsHighlight = 1");
            strSql.Append(" AND a.DeleteMark = 0");
            var parameter = new List<DbParameter>();
            parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "userId", userId));
            if (!keyword.IsEmpty())
        ...
        public IEnumerable<EmailContentEntity> GetRecycleMail(Pagination pagination, string userId, string keyword)
        {
            ...
            strSql.Append(@"SELECT  *
                            FROM    ( SELECT    a.AddresseeId AS ContentId ,
                                      ...
                                      FROM      Email_Addressee a
                                                LEFT JOIN Email_Content c ON c.ContentId = a.ContentId
                                      WHERE     a.RecipientId = " + DbParameters.CreateDbParmCharacter() + "userId");
            strSql.Append(@"                    AND a.DeleteMark = 1
                                      UNION
                                      SELECT    c.ContentId ,
                                                c.Theme ,
                                                c.ThemeColor ,
                                      ...
                                      WHERE     c.CreateUserId = " + DbParameters.CreateDbParmCharacter() + "userId  AND c.DeleteMark = 1) t WHERE 1=1");
            var parameter = new List<DbParameter>();
            parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "userId", userId));
            if (!keyword.IsEmpty())
            {
                strSql.Append(" AND Theme like " + DbParameters.CreateDbParmCharacter() + "Theme");
                parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "Theme", '%' + keyword + '%'));
            }
            return this.BaseRepository().FindList(strSql.ToString(), parameter.ToArray(), pagination);
        }
        ...
        public int GetRecycleMailCount(string userId)
        {
            ...
            strSql.Append(@"                    AND a.DeleteMark = 1
                                      ...
                                      SELECT    c.ContentId ,
                                                ...
                                                c.CreateDate
                                      FROM      Email_Content c
                                      WHERE     c.CreateUserId = " + DbParameters.CreateDbParmCharacter() + "userId AND c.DeleteMark = 1) t");
            var parameter = new List<DbParameter>();
            parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "userId", userId));
