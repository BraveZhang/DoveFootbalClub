...
using DOVE.Application.Entity.AuthorizeManage.ViewModel;
                            ...
                            WHERE   ModuleId IN (
                                    ...
                                    WHERE   ItemType = 1
                                            AND (( ObjectId IN (
                                                  ...
                                                  WHERE     UserId = {0} ) )
                                            OR ObjectId = {1} ))
                            AND EnabledMark = 1  AND DeleteMark = 0 Order By SortCode", DbParameters.CreateDbParmCharacter() + "UserId", DbParameters.CreateDbParmCharacter() + "UserId");
            DbParameter[] parameter =
            {
                DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter()+"UserId",userId)
            };
                            ...
                            WHERE   ModuleButtonId IN (
                                    ...
                                    WHERE   ItemType = 2
                                            AND ( ObjectId IN (
                                                  ...
                                                  FROM      Base_UserRelation
                                                  WHERE     UserId = {0} ) )
                                            OR ObjectId = {1} ) Order By SortCode", DbParameters.CreateDbParmCharacter() + "UserId", DbParameters.CreateDbParmCharacter() + "UserId");
            DbParameter[] parameter =
            {
                DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter()+"UserId",userId)
            };
        ...
        public IEnumerable<AuthorizeUrlModel> GetUrlList(string userId)
        {
            ...
            strSql.AppendFormat(@"SELECT  ModuleId AS AuthorizeId ,
                            ...
                            WHERE   ModuleButtonId IN (
                                    ...
                                    , DbParameters.CreateDbParmCharacter() + "UserId"
                                    , DbParameters.CreateDbParmCharacter() + "UserId"
                                    , DbParameters.CreateDbParmCharacter() + "UserId"
                                    , DbParameters.CreateDbParmCharacter() + "UserId");
            DbParameter[] parameter =
            ...
            if (isWrite)
            {
                strAuthorData = @"   SELECT    *
                                        ...
                                        ObjectId IN (
                                                SELECT  ObjectId
                                                FROM    Base_UserRelation
                                                WHERE   UserId =" + DbParameters.CreateDbParmCharacter() + "UserId)";
            }
            else
            {
                strAuthorData = @"   SELECT    *
