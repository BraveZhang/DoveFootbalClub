...
using DOVE.Application.IService.ReportManage;
using DOVE.Data;
...
namespace DOVE.Application.Service.ReportManage
{
    /// <summary>
    /// 版 本 6.1
    ...
    public class RptTempService : RepositoryFactory<RptTempEntity>, IRptTempService
    {
        ...
        public IEnumerable<RptTempEntity> GetList(string queryJson)
        {
            ...
            if (!queryParam["condition"].IsEmpty() && !queryParam["keyword"].IsEmpty())
            {
                ...
                {
                    case "EnCode":            //角色编号
                        strSql.Append(" AND r.EnCode LIKE " + DbParameters.CreateDbParmCharacter() + "keyword ");
                        parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "keyword", '%' + keyword + '%'));
                        break;
                    case "FullName":          //角色名称
                        strSql.Append(" AND r.FullName LIKE " + DbParameters.CreateDbParmCharacter() + "keyword ");
                        parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "keyword", '%' + keyword + '%'));
                        break;
                    default:
                        break;
            ...
            {
                strSql.Append(" AND r.TempCategory = " + DbParameters.CreateDbParmCharacter() + "TempCategory ");
                parameter.Add(DbParameters.CreateDbParameter(DbParameters.CreateDbParmCharacter() + "TempCategory", queryParam["reportCode"].ToString()));
            }
