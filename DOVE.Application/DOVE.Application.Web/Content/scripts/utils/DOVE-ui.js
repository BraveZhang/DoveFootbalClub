...
$.fn.authorizeColModel = function () {
    ...
    $.each(columnModel, function (i) {
        ...
        if (columnModel[i].name != "rn" && columnModel[i].name != "subgrid") {// modify by zy 20171023 修复有子grid的情况下会导致+号隐藏的bug
...
// add by zy 20170926 可以自定义时间
dialogMsg = function (content, type, times) {
    if (type == -1) {
        type = 2;
    }
    top.layer.msg(content, { icon: type, time: times, shift: 5 });
}
