...
namespace DOVE.Data.EF
{
    ...
    public class Database : IDatabase
    {
        ...
        public Database(string connString, string DbType)
        {
            if (DbType == "")
            {
                dbcontext = (DbContext)UnityIocHelper.DBInstance.GetService<IDbContext>(new ParameterOverride("connString", connString));
            }
            else
            {
                dbcontext = (DbContext)UnityIocHelper.DBInstance.GetService<IDbContext>(DbType, new ParameterOverride("connString", connString));
            }
        ...
        public IEnumerable<T> FindList<T>(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total) where T : class
        {
            ...
            // add by zy 20170914 多数据库语句支持
            switch (DbHelper.DbType)
            {
                case DatabaseType.SqlServer:
                    return FindListBySqlServer<T>(strSql, dbParameter, orderField, isAsc, pageSize, pageIndex, out total);
                case DatabaseType.Oracle:
                    return FindListByOracle<T>(strSql, dbParameter, orderField, isAsc, pageSize, pageIndex, out total);
                default:
                    throw new Exception("数据库类型目前不支持！");
            }
        }

        private IEnumerable<T> FindListByOracle<T>(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total) where T : class
        {
            using (var dbConnection = dbcontext.Database.Connection)
            {
                StringBuilder sb = new StringBuilder();
                ...
                sb.Append("select * from(select t.*,rownum rn from(" + strSql + OrderBy + ") t where rownum<" + num3 + ") where rn>" + num + "");
                total = Convert.ToInt32(new DbHelper(dbConnection).ExecuteScalar(CommandType.Text, "Select Count(1) From (" + strSql + ") t", dbParameter));
                IDataReader dr = new DbHelper(dbConnection).ExecuteReader(CommandType.Text, sb.ToString(), dbParameter);
                return ConvertExtension.IDataReaderToList<T>(dr);
            }
        }

        private IEnumerable<T> FindListBySqlServer<T>(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total) where T : class
        {
            using (var dbConnection = dbcontext.Database.Connection)
            {
                StringBuilder sb = new StringBuilder();
        ...
        public DataTable FindTable(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            ...
            {
                case DatabaseType.SqlServer:
                    return FindTableBySqlServer(strSql, dbParameter, orderField, isAsc, pageSize, pageIndex, out total);
                case DatabaseType.Oracle:
                    return FindTableByOracle(strSql, dbParameter, orderField, isAsc, pageSize, pageIndex, out total);
                default:
                    throw new Exception("数据库类型目前不支持！");
            }
        }
        ...
        /// <summary>
        /// 多数据库语句支持Oracle add by zy 20170914 
        /// </summary>
        ...
        /// <returns></returns>
        private DataTable FindTableByOracle(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            using (var dbConnection = dbcontext.Database.Connection)
            {
                StringBuilder sb = new StringBuilder();
                ...
                if (!string.IsNullOrEmpty(orderField))
                {
                    ...
                    //OrderBy = " Order By (select 0 from dual)";
                    OrderBy = "";// add by zy 20171012 防止不适用自己的sql
                sb.Append("select * from(select t.*,rownum rn from(" + strSql + OrderBy + ") t where rownum<" + num3 + ") where rn>" + num + "");
                total = Convert.ToInt32(new DbHelper(dbConnection).ExecuteScalar(CommandType.Text, "Select Count(1) From (" + strSql + ") t", dbParameter));
                IDataReader dr = new DbHelper(dbConnection).ExecuteReader(CommandType.Text, sb.ToString(), dbParameter);
                DataTable resultTable = ConvertExtension.IDataReaderToDataTable(dr);
                return resultTable;
            }
        }

        /// <summary>
        /// 多数据库语句支持SqlServer add by zy 20170914 
        /// </summary>
        /// <param name="strSql"></param>
        /// <param name="dbParameter"></param>
        /// <param name="orderField"></param>
        ...
        /// <param name="total"></param>
        /// <returns></returns>
        private DataTable FindTableBySqlServer(string strSql, DbParameter[] dbParameter, string orderField, bool isAsc, int pageSize, int pageIndex, out int total)
        {
            using (var dbConnection = dbcontext.Database.Connection)
            {
                StringBuilder sb = new StringBuilder();
                ...
                {
                    //OrderBy = "order by (select 0)";
                    OrderBy = "";// add by zy 20171012 防止自定义的sql报错
                }
                sb.Append("Select * From (Select ROW_NUMBER() Over (" + OrderBy + ")");
                sb.Append(" As rowNum, * From (" + strSql + ") As T ) As N Where rowNum > " + num + " And rowNum <= " + num1 + "");
                total = Convert.ToInt32(new DbHelper(dbConnection).ExecuteScalar(CommandType.Text, "Select Count(1) From (" + strSql + ") t", dbParameter));
