...
namespace DOVE.CodeGenerator
{
    ...
    public class CreateCodeFile
    {
        ...
        public static void CreateExecution(BaseConfigModel baseConfigModel, string strCode)
        {
            ...
            // add by zy 20170918 增加Oracle层映射类
            string entitymapCode_Oracle = strParam["entitymapCode_Oracle"].ToString();
            string entitymapPath_Oracle = baseConfigModel.OutputMap_Oracle + "\\" + baseConfigModel.OutputAreas + "\\" + baseConfigModel.MapClassName + ".cs";
            if (!System.IO.File.Exists(entitymapPath_Oracle))
            {
                DirFileHelper.CreateFileContent(entitymapPath_Oracle, entitymapCode_Oracle);
            }
            #endregion
